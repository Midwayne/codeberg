...
describe('assertFetchableUrl', () => {
  ...
  it('accepts http(s) public URLs and returns a URL', () => {
    expect(assertFetchableUrl('https://example.com/a', pub).hostname).toBe('example.com');
  ...
  it('rejects private/loopback hosts unless allowed', () => {
    ...
    expect(() => assertFetchableUrl('http://169.254.169.254/latest', pub)).toThrow(/private/);
    expect(assertFetchableUrl('http://localhost:8080', { allowPrivate: true }).hostname).toBe(
      'localhost',
    );
  });
  ...
  it('always blocks cloud metadata hostnames', () => {
    expect(() =>
      assertFetchableUrl('http://metadata.google.internal/x', {
        allowPrivate: true,
      }),
