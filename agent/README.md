...
`codeberg-web` reads a few more of its own. The `codeberg`
...
```sh
codeberg learning search-learning "prior issue"
codeberg learning search-knowledge "service ownership"
...
provenance, retries, and dataset-labeling behavior.

## Web

The interactive surface is a React chat UI (ai-sdk `useChat` +
...
`codeberg-web [provider:model]` takes no question argument;
...
Codeberg prompt hooks (`/enhance` today) rewrite the prompt before the tool loop
runs. They work in both the CLI and web UI; the web UI also provides autocomplete.
...
Works the same from the CLI or web composer — the hook rewrites what the model
...
UI wiring required, and it works from the CLI immediately because users only
need to type the trigger.
...
8. **`find_references`** (daemon) — graph-first usages (`{source, graph|matches}`);
   word-boundary grep fallback.
9. **`detect_changes` / `get_architecture`** (daemon) — hunk-filtered blast radius
   and repo structural overview (hubs, languages, entrypoints).
